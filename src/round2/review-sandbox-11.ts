export interface ReviewRecord11 {
  readonly id: number;
...
export class ReviewSandbox11 {
  private records: StoredReviewRecord11[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord11 {
    const record: ReviewRecord11 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord11, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord11[]> {
    const groups = new Map<string, ReviewRecord11[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord11 = {
        ...record,
