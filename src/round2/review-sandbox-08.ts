export interface ReviewRecord08 {
  readonly id: number;
...
export class ReviewSandbox08 {
  private records: StoredReviewRecord08[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord08 {
    const record: ReviewRecord08 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord08, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord08[]> {
    const groups = new Map<string, ReviewRecord08[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord08 = {
        ...record,
