export interface ReviewRecord12 {
  readonly id: number;
...
export class ReviewSandbox12 {
  private records: StoredReviewRecord12[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord12 {
    const record: ReviewRecord12 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord12, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord12[]> {
    const groups = new Map<string, ReviewRecord12[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord12 = {
        ...record,
