export interface ReviewRecord04 {
  readonly id: number;
...
export class ReviewSandbox04 {
  private records: StoredReviewRecord04[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord04 {
    const record: ReviewRecord04 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord04, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord04[]> {
    const groups = new Map<string, ReviewRecord04[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord04 = {
        ...record,
