export interface ReviewRecord06 {
  readonly id: number;
...
export class ReviewSandbox06 {
  private records: StoredReviewRecord06[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord06 {
    const record: ReviewRecord06 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord06, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord06[]> {
    const groups = new Map<string, ReviewRecord06[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord06 = {
        ...record,
