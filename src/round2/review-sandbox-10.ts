export interface ReviewRecord10 {
  readonly id: number;
...
export class ReviewSandbox10 {
  private records: StoredReviewRecord10[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord10 {
    const record: ReviewRecord10 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord10, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord10[]> {
    const groups = new Map<string, ReviewRecord10[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord10 = {
        ...record,
