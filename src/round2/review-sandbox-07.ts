export interface ReviewRecord07 {
  readonly id: number;
...
export class ReviewSandbox07 {
  private records: StoredReviewRecord07[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord07 {
    const record: ReviewRecord07 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord07, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord07[]> {
    const groups = new Map<string, ReviewRecord07[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord07 = {
        ...record,
