export interface ReviewRecord09 {
  readonly id: number;
...
export class ReviewSandbox09 {
  private records: StoredReviewRecord09[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord09 {
    const record: ReviewRecord09 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord09, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord09[]> {
    const groups = new Map<string, ReviewRecord09[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord09 = {
        ...record,
