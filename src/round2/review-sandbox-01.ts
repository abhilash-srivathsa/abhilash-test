export interface ReviewRecord01 {
  id: number;
  team: string;
  author: string;
  body: string;
...
export class ReviewSandbox01 {
  private records: ReviewRecord01[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord01 {
    const record: ReviewRecord01 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  groupByTeam(): Record<string, ReviewRecord01[]> {
    const groups: Record<string, ReviewRecord01[]> = {};
    for (const record of this.records) {
