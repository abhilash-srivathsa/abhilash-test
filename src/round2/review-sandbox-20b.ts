export interface ReviewSnapshot20B {
  id: number;
  lane: string;
  actor: string;
  payload: string;
...
export class ReviewSandbox20B {
  ...
  collectByLane(): Record<string, ReviewSnapshot20B[]> {
    const buckets: Record<string, ReviewSnapshot20B[]> = {};
    for (const snapshot of this.snapshots) {
      if (!buckets[snapshot.lane]) {
        buckets[snapshot.lane] = [];
      }
      buckets[snapshot.lane].push(snapshot);
    }
    return buckets;
  }
