export interface ReviewRecord05 {
  readonly id: number;
...
export class ReviewSandbox05 {
  private records: StoredReviewRecord05[] = [];
  private nextId = 1;

  createRecord(team: string, author: string, body: string): ReviewRecord05 {
    const record: ReviewRecord05 = {
      id: this.nextId++,
      team,
      author,
      body,
  ...
  overwriteMatches(
    matcher: Readonly<Partial<Pick<ReviewRecord05, 'id' | 'team' | 'author' | 'body'>>>,
    nextBody: string
  ): number {
  ...
  groupByTeam(): Record<string, ReviewRecord05[]> {
    const groups = new Map<string, ReviewRecord05[]>();
    for (const record of this.records) {
      const snapshot: ReviewRecord05 = {
        ...record,
